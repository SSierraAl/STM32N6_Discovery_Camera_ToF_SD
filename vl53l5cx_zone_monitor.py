...
class Sensor:
    def __init__(self, name, color, tab_widget):
        ...
        self.p3.addItem(pg.InfiniteLine(pos=THRESHOLD_PCT, angle=0,
            pen=pg.mkPen(RED, width=1, style=QtCore.Qt.DashLine),
            label=f'Drop% > {THRESHOLD_PCT} (VL53L5CX_DET_THRESHOLD_PCT)',
            labelOpts={'color': RED, 'position': 0.95}))
        self.p4 = self._plot("Motion",           0, 100)
        self.p4.addItem(pg.InfiniteLine(pos=MOTION_THRESH, angle=0,
            pen=pg.mkPen(ORANGE, width=1, style=QtCore.Qt.DashLine),
            label=f'Motion >= {MOTION_THRESH} (VL53L5CX_DET_MOTION_THRESH)',
        ...
        self.lbl_host = QtWidgets.QLabel(f"host: 0/{MIN_AFFECTED_ZONES} zones (no trigger)")
    ...
    def update(self, zones, motion):
        ...
        sig_hit = [z for z in range(NUM_ZONES)
                   if zones[z]['sig'] >= MIN_SIGNAL and zones[z]['drop'] > THRESHOLD_PCT]
        mot_hit = [z for z in range(NUM_ZONES)
                   if (motion[z] if z < len(motion) else 0) >= MOTION_THRESH]
        host_hit = sorted(set(sig_hit) | set(mot_hit))
        if host_hit and len(host_hit) >= MIN_AFFECTED_ZONES:
        ...
        if host_hit:
            ...
            self.lbl_host.setText(f"host: {len(host_hit)}/{MIN_AFFECTED_ZONES} zones {shown}")
            ...
            self.lbl_host.setText(f"host: 0/{MIN_AFFECTED_ZONES} zones (no trigger)")
            self.lbl_host.setStyleSheet(f"color:{TXT};background:{PLOT_BG};font-family:Consolas;font-size:10pt;padding:4px;border-radius:3px")
...
print(f"Host overlay thresholds (mirror app_config.h SECTION 9): drop>{THRESHOLD_PCT}% sig>={MIN_SIGNAL} motion>={MOTION_THRESH} zones>={MIN_AFFECTED_ZONES}")
