...
def find_latest_datalog():
    ...
    for pattern in patterns:
        # Exclude *_summary.csv session reports (plain-text stats, not datalogs)
        files = [f for f in glob.glob(pattern)
                 if not os.path.basename(f).endswith('_summary.csv')]
        if files:
...
def load_datalog(filepath):
    ...
    try:
        ...
        print(f"  ERROR: '{os.path.basename(filepath)}' is not a parseable datalog CSV.")
        print(f"  ({e})")
        print("  Note: *_summary.csv files are end-of-session reports, not datalogs —")
        print("  pass the base tof_datalog_*.csv file instead.")
        sys.exit(1)
...
def create_report_plots(df, zone_data, stats_df, num_zones, grid_size, output_folder,
    ...
    # Display transform identical to datalogger.py / vl53l5cx_zone_monitor.py
    # (FLIP_V=True): flip the rows before showing, so Z0..Z3 end up on TOP
    # (Z0 top-left). matplotlib with origin='lower' draws row 0 at the
    # bottom, which cancels the flip — the same role the pyqtgraph
    # transpose plays in the datalogger. Cell (i, j) of the flipped array
    # holds zone (grid_size-1-i)*grid_size+j and is centered at (j, i).
    ...
    for i in range(grid_size):
        for j in range(grid_size):
            val = heatmap[i, j]
            ax.text(j, i, f'Z{(grid_size-1-i)*grid_size+j}\n{val:.0f}', ha='center', va='center',
    ...
    heatmap = heatmap[::-1, :].copy()   # FLIP_V, same as datalogger
    im = ax.imshow(heatmap, cmap='plasma', aspect='equal', origin='lower')
    ...
    heatmap = heatmap[::-1, :].copy()   # FLIP_V, same as datalogger
    im = ax.imshow(heatmap, cmap='RdYlGn_r', aspect='equal', origin='lower')
    ...
    heatmap = heatmap[::-1, :].copy()   # FLIP_V, same as datalogger
    im = ax.imshow(heatmap, cmap='hot', aspect='equal', origin='lower')
    plt.colorbar(im, label='Mean Motion Indicator')
    vmin, vmax = float(np.min(heatmap)), float(np.max(heatmap))
    for i in range(grid_size):
        for j in range(grid_size):
            val = heatmap[i, j]
            # imshow autoscales 'hot' over [vmin, vmax]: black at the low end,
            # white at the high end. Pick text color from the cell's
            # normalized intensity so labels stay readable on both ends.
            t = (val - vmin) / (vmax - vmin) if vmax > vmin else 0.5
            ax.text(j, i, f'Z{(grid_size-1-i)*grid_size+j}\n{val:.0f}', ha='center', va='center',
                    color='black' if t > 0.5 else 'white', fontsize=8)
    ...
    if 'motion_g1' in df.columns and (df['motion_g1'].abs().sum() > 0 or df['motion_g2'].abs().sum() > 0):
        ...
        if 'motion_g1' in df.columns:
            print("  [14/14] Skipped: MOTION globals present but all zero (no motion plugin activity in this session)")
        else:
...
def main():
    ...
    # Create output folder (before the sweep table is written into it)
