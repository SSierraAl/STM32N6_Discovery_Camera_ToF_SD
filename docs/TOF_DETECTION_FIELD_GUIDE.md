# 4×4 ToF detection and field calibration

## Scope and version

This guide describes the **single sensor, 4×4, high sensitivity camera** path in
...
The code also supports test mode, legacy camera mode, 8×8 and dual sensor mode.
...
detector settings also affect the equivalent 4×4 `HIGH_SENS_TEST` path.
...
3. A local signal change of at least 3%, or a local distance change of at
   ...
   4×4 high sensitivity camera detector.
...
| `VL53L5CX_DET_BASELINE_SAMPLES` (4×4) | 20 frames | Averages more readings, longer pause, may learn a moving object | Faster baseline, less averaging |
...
I²C and console time. Missed frames/timeouts can extend this pause.
...
| --- | ---: | --- |
| `VL53L5CX_DET_INTEGRATION_MS` (4×4) | 45 ms | Longer sensing window can improve signal; must fit chosen ranging period. Keep 45 ms while calibrating detection. |
...
| `VL53L5CX_DET_THRESHOLD_PCT` (4×4) | 3% | Immediate local signal level. Lower catches weak objects, risks one-frame noise. |
...
| `VL53L5CX_DET_MOTION_THRESH` | 35 | ST motion indicator threshold used by drift/stability logic in this 4×4 path; it is **not** a direct photo trigger. |
