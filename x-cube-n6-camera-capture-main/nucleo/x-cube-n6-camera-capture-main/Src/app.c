...
/* One-shot frame callback for SD card capture with frame skip */
typedef void (*frame_cb_t)(uint8_t *data, int len, int w, int h, int payload_type);
static frame_cb_t g_frame_callback  = NULL;
static int        g_frame_skip_count = 0;  /* skip N frames before triggering */

void APP_SetFrameCallback(frame_cb_t cb, int skip_frames)
{
  g_frame_callback   = cb;
  g_frame_skip_count = skip_frames;
}
...
/* capture buffers (only needed when UVC streaming is active) */
...
static void uvc_thread_fct(void *arg)
{
  ...
  while (1) {
    ...
    while (1) {
      ...
      {
        /* Per-frame callback after skipping N warmup frames (lets camera stabilize)
           The callback is called on EVERY frame after warmup. It internally checks
           snapshot_requested to decide whether to save. */
        if (g_frame_skip_count > 0) {
          g_frame_skip_count--;
        } else if (g_frame_callback) {
          g_frame_callback(buffer->buffer, buffer->len, current.stream.width,
                           current.stream.height, current.stream.payload_type);
        }
...
void app_run()
{
  ...
  /* Initialize FileX and SD card */
  if (MX_FileX_Init() == FX_SUCCESS) {
    APP_SDCard_Init();
    printf("[APP] ✓ FileX and SD card initialized\n");
  } else {
    printf("[APP] ✗ FileX initialization failed - SD storage unavailable\n");
  }
...
int CMW_CAMERA_PIPE_VsyncEventCallback(uint32_t pipe)
{
  if (pipe == DCMIPP_PIPE1)
  {
    /* Always count vsync frames for standalone warmup */
    CAM_CountVsyncFrame();

    /* Guard: isp_sem may be NULL when camera is used standalone
       (without app_run / UVC streaming). Only signal if valid. */
    if (isp_sem != NULL)
