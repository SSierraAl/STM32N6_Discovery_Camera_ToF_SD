...
# Log folder sits next to THIS script, so the toolset works no matter
# which directory you launch it from.
DATA_FOLDER = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'datalog')
...
# ==================================================================
# KEEP IN SYNC with FLIP_V / FLIP_H / COLOR_TRANSPOSE in
# vl53l5cx_zone_monitor.py — the datalogger heatmaps must display the
# same orientation as that monitor.
# Current monitor setting (FLIP_V=True, FLIP_H=False): Z0 top-left,
# the z=0..3 row at the TOP, Z12..Z15 at the bottom.
FLIP_V          = True   # top <-> bottom flip of the zone matrix
FLIP_H          = False  # left <-> right flip of the zone matrix
# pyqtgraph 0.14: ImageItem's default mode ("col-major") draws the
# array's FIRST axis horizontally, so the [row][col] matrix must be
# transposed before setImage. Keep True.
...
# Add zone labels — positions are flipped with the same FLIP_V / FLIP_H
# logic as the image below (and as the Z-ID overlay in
# vl53l5cx_zone_monitor.py), so each Z label sits on its own cell for
# any active orientation.
for row in range(GRID_SIZE):
    for col in range(GRID_SIZE):
        z = row * GRID_SIZE + col
        disp_row = (GRID_SIZE - 1 - row) if FLIP_V else row
        disp_col = (GRID_SIZE - 1 - col) if FLIP_H else col
        font_size = 9 if GRID_SIZE <= 4 else 7
        text = pg.TextItem(f"Z{z}", color=(155, 155, 155), anchor=(0.5, 0.5))
        text.setPos(disp_col + 0.5, disp_row + 0.5)
...
tabs.addTab(tab3, "Session Control")
...
def update_data():
    global frame_count, serial_buffer, logging_active
    ...
    try:
        ...
        while '\n' in serial_buffer:
            ...
            # Same orientation as vl53l5cx_zone_monitor.py: apply FLIP_V /
            # FLIP_H to the [row][col] zone maps first, then COLOR_TRANSPOSE
            # (pyqtgraph 0.14 ImageItem draws the array's FIRST axis
            # horizontally, so transpose before setImage). With the current
            # flags (FLIP_V=True): Z0 at top-left, zones 0..3 along the top.
            if FLIP_V:
                signal_map = signal_map[::-1, :].copy()
                dist_map = dist_map[::-1, :].copy()
            if FLIP_H:
                signal_map = signal_map[:, ::-1].copy()
                dist_map = dist_map[:, ::-1].copy()
            if COLOR_TRANSPOSE:
                signal_map = signal_map.T.copy()
                dist_map = dist_map.T.copy()
            ...
            # Update motion heatmap (same FLIP_V / FLIP_H / COLOR_TRANSPOSE
            # pipeline as the signal and distance maps above)
            ...
            if FLIP_V:
                motion_map = motion_map[::-1, :].copy()
            if FLIP_H:
                motion_map = motion_map[:, ::-1].copy()
            if COLOR_TRANSPOSE:
                motion_map = motion_map.T.copy()
...
def on_export():
    ...
    print(f"\n[Export] Report: {report or 'N/A'} | Frames: {frame_count} | Samples/zone: {samples}")
...
def on_stop():
    ...
    print(f"\n[Stop] Logging stopped - Total frames: {frame_count}")
    print(f"[Stop] Data file:      {data_file or 'N/A'}")
    print(f"[Stop] Summary report: {report or 'N/A'}")
    print("[Stop] Application will close in 3 seconds...")
